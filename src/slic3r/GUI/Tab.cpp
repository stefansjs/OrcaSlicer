...
void TabFilament::build()
{
    ...
    page = add_options_page(L("Cooling"), "custom-gcode_cooling_fan"); // ORCA: icon only visible on placeholders
        ...
        optgroup->append_single_option_line("external_bridge_fan_speed"); // ORCA: Add support for separate external bridge fan speed control
...
void TabFilament::toggle_options()
{
    ...
    if (m_active_page->title() == L("Cooling")) {
      ...
      for (auto el : {"overhang_fan_speed", "overhang_fan_threshold", "external_bridge_fan_speed", "internal_bridge_fan_speed"}) // ORCA: Add support for separate external/internal bridge fan speed control
